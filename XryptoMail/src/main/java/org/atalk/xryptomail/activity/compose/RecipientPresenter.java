...
import org.atalk.xryptomail.activity.compose.ComposeCryptoStatus.ComposeCryptoStatusBuilder;
...
import org.atalk.xryptomail.helper.ReplyToParser.ReplyToAddresses;
...
public class RecipientPresenter implements PermissionPingCallback {
    ...
    private static final String STATE_KEY_CURRENT_XRYPTO_MODE = "state:currentXryptoMode";
    ...
    private XryptoMode currentXryptoMode = XryptoMode.UNKNOWN;
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        currentXryptoMode = XryptoMode.valueOf(savedInstanceState.getString(STATE_KEY_CURRENT_XRYPTO_MODE));
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putString(STATE_KEY_CURRENT_XRYPTO_MODE, currentXryptoMode.toString());
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        if (isCryptoConfigured) {
            ...
            boolean showSignOnly = XryptoMail.getOpenPgpSupportSignOnly();
    ...
    public void asyncUpdateCryptoStatus() {
        ...
        final ComposeCryptoStatus composeCryptoStatus = new ComposeCryptoStatusBuilder()
                ...
                .setXryptoMode(XryptoMode.UNKNOWN) // default to unknown on new build
        ...
        try (ExecutorService eService = Executors.newSingleThreadExecutor()) {
            eService.execute(() -> {
                final RecipientAutocryptStatus recipientAutocryptStatus;

                if (cryptoProviderState == CryptoProviderState.OK) {
                    recipientAutocryptStatus = autocryptStatusInteractor
                            .retrieveCryptoProviderRecipientStatus(getOpenPgpApi(), recipientAddresses);
                }
                else {
                    recipientAutocryptStatus = null;
                }
    ...
    // need to set directly as cachedCryptoStatus is async build
    public void setXryptoMode(XryptoMode xryptoMode) {
        currentXryptoMode = xryptoMode;
        cachedCryptoStatus.setXryptoMode(xryptoMode);
    }
    ...
    public void showPgpSendError(SendErrorState sendErrorState) {
        switch (sendErrorState) {
            ...
            case KEY_CONFIG_ERROR:
                ...
                break;
            default:
                throw new AssertionError("not all error states handled, this is a bug!" + sendErrorState.toString());
        }
    }
    ...
    private void cryptoProviderBindOrCheckPermission() {
        ...
        if (pendingUserInteractionIntent != null) {
            recipientMvpView.launchUserInteractionPendingIntent(pendingUserInteractionIntent, OPENPGP_USER_INTERACTION);
            pendingUserInteractionIntent = null;
            return;
        }
    ...
    public void onPgpPermissionCheckResult(Intent result) {
        ...
        switch (resultCode) {
            ...
            case OpenPgpApi.RESULT_CODE_USER_INTERACTION_REQUIRED:
                ...
                pendingUserInteractionIntent = IntentCompat.getParcelableExtra(result, OpenPgpApi.RESULT_INTENT, PendingIntent.class);
                setCryptoProviderState(CryptoProviderState.ERROR);
                break;
    ...
    private void handleOpenPgpError(OpenPgpError error) {
        ...
        if (error.getErrorId() == OpenPgpError.INCOMPATIBLE_API_VERSIONS) {
            ...
            openPgpProvider = null;
            XryptoMail.setOpenPgpProvider(null);
    ...
    private boolean checkAndIncrementPgpInlineDialogCounter() {
        int pgpInlineDialogCounter = XryptoMail.getPgpInlineDialogCounter();
        if (pgpInlineDialogCounter < PGP_DIALOG_DISPLAY_THRESHOLD) {
            XryptoMail.setPgpInlineDialogCounter(pgpInlineDialogCounter + 1);
            XryptoMail.saveSettingsAsync();
            return true;
        }
        return false;
    }
    ...
    private boolean checkAndIncrementPgpSignOnlyDialogCounter() {
        int pgpSignOnlyDialogCounter = XryptoMail.getPgpSignOnlyDialogCounter();
        if (pgpSignOnlyDialogCounter < PGP_DIALOG_DISPLAY_THRESHOLD) {
            XryptoMail.setPgpSignOnlyDialogCounter(pgpSignOnlyDialogCounter + 1);
            XryptoMail.saveSettingsAsync();
            return true;
        }
        return false;
