...
import org.atalk.xryptomail.activity.setup.AccountSetupContract.View;
...
import org.atalk.xryptomail.mail.autoconfiguration.AutoConfigure.ProviderInfo;
...
public class AccountSetupPresenter implements AccountSetupContract.Presenter, Oauth2PromptRequestHandler
{
    ...
    private class CheckOutgoingTask extends CheckAccountTask
    {
                ...
                throws Exception
        {
            ...
            transport = TransportProvider.getInstance().getTransport(context, accountConfig, Globals.getOAuth2TokenProvider());
            transport.close();
            try {
                transport.open();
            } finally {
    ...
    private void setupFolderNames(String domain)
    {
        ...
        if (domain.endsWith(".yahoo.com")) {
            ...
            accountConfig.setSpamFolderName(XryptoMail.getResString(R.string.special_mailbox_name_spam));
        }
    }
    ...
    private String errorMessageForCertificateException(CertificateValidationException e)
    {
        switch (e.getReason()) {
            ...
            case MissingCapability:
                return XryptoMail.getResString(R.string.auth_external_error);
            case RetrievalFailure:
                return XryptoMail.getResString(R.string.client_certificate_retrieval_failure, e.getAlias());
            case UseMessage:
                return e.getMessage();
            case Unknown:
            default:
    ...
    private void handleCertificateValidationException(CertificateValidationException cve)
    {
        ...
        if (chain != null) {
            ...
            view.showErrorDialog(R.string.account_setup_failed_dlg_server_message_fmt, errorMessageForCertificateException(cve));
        }
    }
    ...
    private static class Provider implements Serializable
    {
                ...
                throws URISyntaxException
        {
            ...
            provider.incomingUriTemplate = new URI(providerInfo.incomingType + "+"
                    ...
                    null, providerInfo.incomingAddr, providerInfo.incomingPort, null, null, null);
            provider.outgoingUriTemplate = new URI(providerInfo.outgoingType + "+"
                    ...
                    null, providerInfo.outgoingAddr, providerInfo.outgoingPort, null, null, null);
            return provider;
        }
    }
            ...
            boolean subscribedFoldersOnly)
    {
        ...
        if (Type.IMAP == incomingSettings.type) {
            extra = new HashMap<>();
            extra.put(ImapStoreSettings.AUTODETECT_NAMESPACE_KEY, Boolean.toString(autoDetectNamespace));
            extra.put(ImapStoreSettings.PATH_PREFIX_KEY, imapPathPrefix);
        }
        else if (WebDAV == incomingSettings.type) {
            extra = new HashMap<>();
            extra.put(WebDavStoreSettings.PATH_KEY, webdavPathPrefix);
            extra.put(WebDavStoreSettings.AUTH_PATH_KEY, webdavAuthPath);
            extra.put(WebDavStoreSettings.MAILBOX_PATH_KEY, webdavMailboxPath);
        }
    ...
    private void updateAccount()
    {
        ...
        try {
            ...
            isPushCapable = remoteStore.isPushCapable();
        } catch (Exception e) {
