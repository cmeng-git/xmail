...
import org.atalk.xryptomail.mailstore.migrations.*;
   ...
   private final LocalStore localStore;
    ...
    public void doDbUpgrade(final SQLiteDatabase db) {
        try {
            ...
            // cmeng - 8 May 2016
            int mVersion_org = db.getVersion();
            db.setVersion(mVersion_org);
            upgradeDatabase(db);
        }
    }

    private void upgradeDatabase(final SQLiteDatabase db) {
        Timber.i("Upgrading database from version %d to version %d", db.getVersion(), LocalStore.DB_VERSION);
    ...
    private static void dbCreateDatabaseFromScratch(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE messages (" +
                ...
                "stealth_timer INTEGER default 30" +
                ")");
