...
class ImapResponseParser
{
    ...
    private void readContinuationRequest(ImapResponseCallback callback)
            throws IOException
    {
    ...
    private void readUntaggedResponse(ImapResponseCallback callback)
            throws IOException
    {
    ...
    private void readTaggedResponse(ImapResponseCallback callback)
            throws IOException
    {
    ...
    List<ImapResponse> readStatusResponse(String tag, String commandToLog, String logId, UntaggedHandler untaggedHandler)
            throws IOException, NegativeImapResponseException
    {
        ...
        do {
            ...
            if (response.isTagged() && !response.getTag().equalsIgnoreCase(tag)) {
            ...
            if (!response.isTagged() && untaggedHandler != null) {
    ...
    private void readTokens(ImapResponse response)
            throws IOException
    {
        response.clear();
     ...
     */
    private void parseResponseText(ImapResponse parent)
            throws IOException
    {
        skipIfSpace();
    ...
    private void parseListResponse(ImapResponse response)
            throws IOException
    {
        expect(' ');
        parseList(response, '(', ')');
     ...
     */
    private Object readToken(ImapResponse response)
            throws IOException
    {
        while (true) {
    ...
    private Object parseToken(ImapList parent)
            throws IOException
    {
        while (true) {
    ...
    private ImapList parseList(ImapList parent, char start, char end)
            throws IOException
    {
        expect(start);
    ...
    private String parseQuotedOrNil()
            throws IOException
    {
        int peek = inputStream.peek();
        if (peek == '"') {
    ...
    private String readStringUntilEndOfLine()
            throws IOException
    {
    ...
    {
        return !((token == null) || !(token instanceof String))
                && symbol.equalsIgnoreCase((String) token);
    }
